...
/>
     ...
     <Row className='google-row'>
        <Col lg={4}></Col>
        <Col className='google-parent' lg={4}>
          <div style={{display:'flex',justifyContent:'center'}}>
          <GoogleLogin 
  onSuccess={credentialResponse => {
    let encoded = jwtDecode(credentialResponse.credential)
    setEmail(encoded)
    console.log(credentialResponse)
  }}
  type="icon"
...
/>
 ...
 export function AppRegister () {
    ...
    return(
        ...
        <Form.Control type="date" name='dob' onChange={(e) => {
          setRegisterdata({
            ...regsiterdata,
            [e.target.name] : e.target.value,
          })
...
export default function AppBuyerlogin() {
